 ...
 *   walking distance (600m) of the DA centroid
...
function main() {
  ...
  const WALK_RADIUS = 0.6 // 600m in km — all stops treated as bus
  ...
  for (const da of das.features) {
    ...
    const [lon, lat] = daCentroid.geometry.coordinates
    ...
    for (const stop of nearby) {
      const dist = distance(daCentroid, stop, { units: 'kilometers' })
      if (dist <= WALK_RADIUS) {
