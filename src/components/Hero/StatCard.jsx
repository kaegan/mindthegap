...
export default function StatCard({ numericValue, suffix, iconName, label, visible }) {
  ...
  return (
    <div className="cs-panel p-6 flex-1 min-w-[200px] border-t-2 border-brand/30">
      {Icon && (
        <div className="mb-3">
          <Icon size={24} className="text-brand/70" />
        </div>
      )}
      <div className="mb-2">
        <AnimatedNumber target={numericValue} suffix={suffix} visible={visible} />
      </div>
      <div className="text-sm text-text-secondary leading-relaxed">{label}</div>
    </div>
  )
}
